...
public class PulseDataLineRenderer : PulseDataConsumer
{
  ...
  override internal void UpdateFromPulse(DoubleList times, DoubleList values)
  {
    ...
    //for (int i = 0; i < SignalFilteringECG.unprocessed_data.GetLength(0); ++i)
    for (int j=0;j<SignalFilteringECG.unprocessed_data.GetLength(0);j++){
      AddPoint((int)SignalFilteringECG.unprocessed_data[0,j],(int)SignalFilteringECG.unprocessed_data[1,j]);
    }
    ...
    for (int i = 0; i < times.Count; ++i)
      ...
      //AddPoint(times.Get(i), values.Get(i));
      AddPoint(times.Get(i), (int)SignalFilteringECG.unprocessed_data[3,i]);
      /*
      for (int j=0;j<SignalFilteringECG.unprocessed_data.GetLength(0);j++){

        AddPoint(times.Get(i), (int)SignalFilteringECG.unprocessed_data[1,j]);
      }
      */
  }
